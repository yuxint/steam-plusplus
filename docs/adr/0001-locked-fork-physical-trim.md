...
- 2026-09-10：P5 迅游游戏加速删除完成（GUI 验收通过后实施）。整删：插件内 XunYouSDK 目录、XY_* 服务接口与实现（IAcceleratorService/BackendAcceleratorServiceImpl 连根删，其唯一职责就是订阅迅游状态）、GameAcceleratorService、GameAcceleratorSettings、游戏加速全套 UI（GameAccelerator/GameAppItem/GameDetail/GameInfoPage/AcceleratorPathAskBox、GameAccelerator ViewModels 目录）、旧版 AcceleratorPage（死页面）、孤儿 NetworkEnvCheckService/NetworkEnvCheckStep。改删：AcceleratorPage2 去"游戏加速"tab、侧栏"Watt 加速器相关"分组、游戏搜索框与 VIP 横幅；Plugin.cs 去迅游初始化/DI 注册/设置项；ProxySettings 删 AutoShowWattAcceleratorWindow 键（MP2Key 19 删除后 ProxyBeforeDNSCheck 20→19 整体重排）。迅游删除后登录栈失去最后调用方，连带整删：UserService、LoginOrRegisterWindowViewModel（+props）、ThirdPartyLoginHelper、LoginOrRegisterPage、SendSmsUIHelper（短信登录辅助，已无使用方）；NavigationService 去登录页映射，App.OpenBrowserCommandCore 去官网免登跳转（退化为直接开浏览器）。resx 清理 21 个死键（GameAccelerator_* 13 个 + 登录相关 8 个，含 Designer.cs 同步）。已知残留：ref/WTTS.MicroServices.ClientSDK 子模块（锁 3.1.0）内仍有休眠的迅游 API 定义（GetXunYouVipEndTime、XunYou* 模型/枚举），无任何调用方，按基线策略不动 ref/ 子模块，维持不裁。
- 2026-09-15：P5 重建与修正。单编插件工程（Release 双 TFM）暴露 P5 首轮验证盲区：插件不在主程序编译图（MEF 动态加载、无 ProjectReference），此前"三端编译"实际未编到插件——ImplicitUsings.cs 三条 global using（Mobius/Mobius.Enums/Mobius.Models）的命名空间提供方正是被删的 XunYouSDK 文件，悬空导致 CS0246，删除三条 using 后插件重编通过（1.83MB→1.52MB）。随后完整重建 .app：publish 主程序（net10.0-macos osx-arm64）→ 重铺 modules/Accelerator 三件套（新插件 DLL + Steam++.Accelerator 单文件 + libe_sqlite3.dylib，MD5 对账一致；子进程源码未动，与 P4 字节数一致属预期）→ MonoBundle 核验（无 AppCenter/WebView2/Facepunch/迅游残留文件，主程序集 09-15 时间戳，Release 裁剪产物对被删类型名二进制扫描零命中）。.app 321MB，待 P5 GUI 验收。
